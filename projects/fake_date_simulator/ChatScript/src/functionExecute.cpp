...
char* DoFunction(char* name,char* ptr,char* buffer,FunctionResult &result) // DoCall(
{
	...
	bool oldecho = echo; 
	if (D->internalBits & MACRO_TRACE) echo = true;
	...
	if (D->x.codeIndex && !(D->internalBits & (IS_PLAN_MACRO|IS_TABLE_MACRO))) // system function --  macroFlags are also on codeindex, but IS_TABLE_MACRO distinguishes  but PLAN also has a topicindex which is a codeindex
	{
		...
		if ((trace & TRACE_OUTPUT || D->internalBits & MACRO_TRACE ) && CheckTopicTrace()) Log(STDUSERTABLOG, "System Call %s(",name);
		...
		while (ptr && *ptr != ')' && *ptr != ENDUNIT) // read arguments
		{
			...
			if ((trace & TRACE_OUTPUT || D->internalBits & MACRO_TRACE) && CheckTopicTrace())  Log(STDUSERLOG,"%s, ",callArgumentList[callArgumentIndex]);
		...
		*callArgumentList[callArgumentIndex] = 0; //  mark end of arg list with null value
		if ((trace & TRACE_OUTPUT  || D->internalBits & MACRO_TRACE) && CheckTopicTrace()) Log(STDUSERLOG,") = ");
	...
	else //   user function (plan macro, inputmacro, outputmacro, tablemacro)) , eg  ^call (_10 ^2 it ^call2 (3 ) )  spot each token has 1 space separator 
	{
		...
		if ((trace & (TRACE_OUTPUT|TRACE_USERFN) || D->internalBits & MACRO_TRACE) && CheckTopicTrace()) Log(STDUSERTABLOG, "Call %s(",name);
		if (!D->w.fndefinition)
		{
        ...
        while (ptr && *ptr && *ptr != ')') //   ptr is after opening (and before an arg but may have white space
        {
			...
			if ((trace & (TRACE_OUTPUT|TRACE_USERFN) || D->internalBits & MACRO_TRACE)  && CheckTopicTrace())
			{
		...
		{
			if ((trace & (TRACE_OUTPUT|TRACE_USERFN) || D->internalBits & MACRO_TRACE) && CheckTopicTrace()) Log(STDUSERLOG, "null, ");
			char* arg = callArgumentList[callArgumentIndex++];
			*arg = 0; // empty string
		}
		if (trace == TRACE_USERFN)  Log(STDUSERLOG, ") => ");
		else if ((trace & (TRACE_OUTPUT|TRACE_USERFN) || D->internalBits & MACRO_TRACE) && CheckTopicTrace()) Log(STDUSERLOG, ")\n");
		fnVarBase = callArgumentBase = oldArgumentIndex; 
	
		//   run the definition
		unsigned int oldtrace = trace;
		if (D->internalBits & MACRO_TRACE) trace = (unsigned int) -1;
...
{ 
	...
	if (block->topicDebug)  
	{
		...
		if (trace) Log(STDUSERLOG,"  try rejoinder for: %s %d.%d",GetTopicName(currentTopicID),TOPLEVELID(inputRejoinderRuleID),REJOINDERID(inputRejoinderRuleID));
	}
...
static int JSONArgs() 
{
	...
	jsonUnique = false;
	if (strstr(arg1,"unique")) used = jsonUnique = true;
	else if (strstr(arg1,"refresh")) { used = true; jsonUnique = false; }
	if (used) ++index;
	return index;
}
...
int buildFactsFromJsonHelper(char *jsontext, jsmntok_t *tokens, int currToken, MEANING *retMeaning, int* flags) {
	...
	*flags = 0;
	...
	case JSMN_PRIMITIVE: { //  true  false, numbers, null 
		(jsontext + curr.start)[len] = 0;
		*retMeaning = MakeMeaning(StoreWord(jsontext + curr.start,AS_IS));
		*flags = JSON_PRIMITIVE_VALUE; // json primitive type
		break;
	}
	case JSMN_STRING: {
		(jsontext + curr.start)[len] = 0;
		if (len == 0)  *retMeaning = MakeMeaning(StoreWord("null",AS_IS));
		else *retMeaning = MakeMeaning(StoreWord(jsontext + curr.start,AS_IS));
		*flags = JSON_STRING_VALUE; // string null
		break;
	}
	case JSMN_OBJECT: {
		// Build the object name
		if (!jsonUnique) sprintf(namebuff, "jo-%d", objectcnt++);
		else sprintf(namebuff, "jo-%d-%d", objectcnt++,volleyCount);
		MEANING objectName = MakeMeaning(StoreWord(namebuff,AS_IS));
		*retMeaning = objectName;
		for (int i = 0; i < curr.size / 2; i++) { // each entry takes an id and a value
			MEANING keyMeaning = 0;
			int flags = 0;
			retToken = buildFactsFromJsonHelper(jsontext, tokens, retToken, &keyMeaning, &flags);
			MEANING valueMeaning = 0;
			retToken = buildFactsFromJsonHelper(jsontext, tokens, retToken, &valueMeaning, &flags);
	...
	case JSMN_ARRAY: {
		...
		if (!jsonUnique) sprintf(namebuff,  "ja-%d", arraycnt++);
		else sprintf(namebuff,  "ja-%d-%d", arraycnt++,volleyCount);
		MEANING arrayName = MakeMeaning(StoreWord(namebuff,AS_IS));
		*retMeaning = arrayName;
		...
		for (int i = 0; i<curr.size; i++) {
			...
			int flags = 0;
			retToken = buildFactsFromJsonHelper(jsontext, tokens, retToken, &arrayMeaning, &flags);
...
MEANING buildFactsFromJson(char *jsontext, jsmntok_t *tokens) {
	MEANING retToken = 0;
	int flags = 0;
	buildFactsFromJsonHelper(jsontext, tokens, 0, &retToken, &flags);
	return retToken;
}
...
static FunctionResult ParseJson(char* buffer, char* message, size_t size)
{
	if (trace & TRACE_JSON) Log(STDUSERTABLOG, "ParseJson Call: %s", message);
...
static FunctionResult JSONCreateCode(char* buffer) 
{
	...
	{
		if (!jsonUnique) sprintf(buffer,  "ja-%d", arraycnt++);
		else sprintf(buffer,  "ja-%d-%d", arraycnt++,volleyCount);
		StoreWord(buffer,AS_IS);
	}
	else if (!stricmp(arg,"object"))
	{
		if (!jsonUnique) sprintf(buffer, "jo-%d", objectcnt++);
		else sprintf(buffer, "jo-%d-%d", objectcnt++,volleyCount);
		StoreWord(buffer,AS_IS);
	}
	else return FAILRULE_BIT;
	return NOPROBLEM_BIT;
...
static FunctionResult JSONDeleteCode(char* buffer) // factstore, fact to delete
{
	jsonStore = GetSetID(ARGUMENT(1)); 
	if (jsonStore == ILLEGAL_FACTSET) return FAILRULE_BIT;
	jsonIndex = 0;
	FACT* F = Index2Fact(atoi(ARGUMENT(2)));
	if (!F || !(F->flags & JSON_FLAGS)) return FAILRULE_BIT;
	
	// need to renumber array facts which may not be in order, make them in order
	if (F->flags & JSON_ARRAY_FACT && JsonArrayRenumber(F) < 0) return FAILRULE_BIT;
	if (trace & TRACE_JSON) TraceFact(F);
	factSet[jsonStore][jsonIndex++] = F;
	jsonGather( Meaning2Word(F->object),F->flags & JSON_FLAGS);

	while (jsonIndex >= 0)  KillFact(factSet[jsonStore][--jsonIndex]);
...
SystemFunctionInfo systemFunctionSet[] =
{
	...
	{ "^jsondelete", JSONDeleteCode, 2, 0, "given scratch factset and json fact, delete fact and all subsidiary facts" },
