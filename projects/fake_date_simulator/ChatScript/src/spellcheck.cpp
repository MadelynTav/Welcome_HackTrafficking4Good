...
void InitSpellCheck()
{
	...
	{
		if (!D->word || !IsAlphaUTF8(*D->word) || D->length >= 100 ) continue; // || D->internalBits & UTF8
...
char* SpellFix(char* originalWord,unsigned int start,uint64 posflags)
{
	...
	for (unsigned int i = 0; i < 3; ++i)
	{
		...
		while (offset)
		{
			...
			if (trace == TRACE_SPELLING) Log(STDUSERLOG,"Test: %s against %s value: %d\r\n",D->word,originalWord,val);
