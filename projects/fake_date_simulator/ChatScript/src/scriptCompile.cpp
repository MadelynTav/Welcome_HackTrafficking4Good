	...
	while (ALWAYS) //   read as many tokens as needed to complete the call, storing them locally
	{
		...
		default:
			...
			if (parenLevel == 1)  
			{
				...
				if (*word == '^' && *nextToken != '(' && !IsDigit(word[1])) WARNSCRIPT("Is %s intended as a function call?",word) //   not function call or function var ref
			...
			{
				ReadNextSystemToken(in,ptr,nextToken,false,true);
				if (*word == '^' && *nextToken != '(' && *nextToken != ')'  && !IsDigit(word[1])) WARNSCRIPT("Is %s intended as a function call?",word) //   not function call or function var ref
			}
		}
